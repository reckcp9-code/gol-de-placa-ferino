...
const PRESIDENT_UI=`<style>#voteCount,.stats,#adminCodes{display:none!important}.president-delete-btn{margin-left:10px!important;background:linear-gradient(135deg,#b51212,#ff4d4d)!important;color:#fff!important;border:2px solid #ff6464!important}</style><script>(()=>{function addDelete(target){if(!target||target.dataset.presidentDeleteReady)return;target.dataset.presidentDeleteReady='1';const b=document.createElement('button');b.className=(target.className||'')+' president-delete-btn';b.textContent='Excluir';b.type='button';b.onclick=async()=>{if(!confirm('Excluir esta partida, seus códigos e todos os votos? Esta ação não pode ser desfeita.'))return;const pin=document.getElementById('pin')?.value||'';try{const r=await fetch('/api/admin/delete',{method:'POST',headers:{'x-admin-pin':pin}});const d=await r.json();if(!r.ok)throw Error(d.error||'Erro ao excluir');alert('Partida excluída com sucesso.');location.reload()}catch(e){alert(e.message)}};target.insertAdjacentElement('afterend',b)}function install(){document.querySelectorAll('#voteCount,.stats,#adminCodes').forEach(e=>e.style.display='none');const open=document.getElementById('openVote');if(open)addDelete(open);document.querySelectorAll('button').forEach(btn=>{const t=(btn.textContent||'').trim().toLocaleLowerCase('pt-BR');if(t==='reabrir partida')addDelete(btn)})}document.addEventListener('DOMContentLoaded',install);new MutationObserver(install).observe(document.documentElement,{childList:true,subtree:true})})();</script>`;
