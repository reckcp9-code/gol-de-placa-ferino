...
export default {async fetch(request,env){
  ...
  try{
    ...
    if(request.method==="GET"&&u.pathname==="/api/status"){
      const m=await getMatch(env),submitted=await countVotes(env,m.id);
      return json({match:{id:m.id,name:m.name,season:m.season,status:m.status},open:m.status==="open",total:(m.codes||[]).length,submitted});
    }
    if(request.method==="GET"&&u.pathname==="/api/ranking"){
      ...
      return json({season,rows:rankingRows(season===2026?SEED_2026:{},await getDynamicRanking(env,season))});
    }
    if(request.method==="GET"&&u.pathname==="/api/results"){
      const m=await getMatch(env);
      if(m.status==="open")return json({error:"Resultado secreto enquanto a votação estiver aberta.",open:true},423);
      const saved=await env.STATE.get(`result:${m.id}`,"json");
      return json({match:{id:m.id,name:m.name,season:m.season},rows:saved?.rows||rowsFromVotes(await loadVotes(env,m.id))});
    }
    if(request.method==="POST"&&u.pathname==="/api/vote"){
      const m=await getMatch(env);
      if(m.status!=="open")return json({error:"A votação já foi encerrada. O resultado está liberado."},423);
      const b=await request.json();
      if(!b.code||!b.votes||!CATS.every(c=>b.votes[c]))return json({error:"Preencha todas as categorias."},400);
      const active=new Set((await getPlayers(env)).filter(p=>p.active).map(p=>p.name));
      ...
      if(!(m.codes||[]).includes(code))return json({error:"Código inválido para esta partida."},400);
      const key=votePrefix(m.id)+code;
      if(await env.STATE.get(key))return json({error:"Este código já foi utilizado nesta partida."},400);
      const votes={};for(const c of CATS)votes[c]=String(b.votes[c]).trim();
      await env.STATE.put(key,JSON.stringify({code,votes,createdAt:new Date().toISOString()}));
      return json({ok:true});
    }
    if(request.method==="GET"&&u.pathname==="/api/admin/status"){
      ...
      const m=await getMatch(env),players=await getPlayers(env),submitted=await countVotes(env,m.id);
      return json({match:{id:m.id,name:m.name,season:m.season,status:m.status},total:(m.codes||[]).length,submitted,players,matches:[{id:m.id,name:m.name,season:m.season,status:m.status}]});
    }
    if(request.method==="POST"&&u.pathname==="/api/admin/matches"){
      ...
      const current=await getStoredMatch(env);
      if(current?.status==="open")return json({error:"Já existe uma partida aberta para votação. Encerre a partida atual antes de abrir outra."},409);
      const season=Math.max(2026,Math.min(2100,Number(b.season)||2026)),m=newMatch(name,season);
      await putMatch(env,m);
      return json({ok:true,id:m.id,name:m.name,season:m.season});
    }
    if(request.method==="POST"&&u.pathname==="/api/admin/codes"){
      if(!admin(request,env))return json({error:"PIN de administrador incorreto."},401);
      let m=await getMatch(env);
      if(m.status!=="open")return json({error:"A votação está encerrada. Crie ou reabra uma partida antes de gerar códigos."},423);
      if(await countVotes(env,m.id))return json({error:"Já existem votos nesta partida. Não é possível trocar os códigos agora."},409);
      const b=await request.json(),total=Math.max(1,Math.min(60,Number(b.total)||20)),codes=generateCodes(total);
      if(m.virtual)m={...newMatch(m.name,m.season),codes};else m={...m,codes};
      await putMatch(env,m);return json({codes,match:m.name});
    }
    if(request.method==="POST"&&u.pathname==="/api/admin/close"){
      if(!admin(request,env))return json({error:"PIN de administrador incorreto."},401);
      let m=await getMatch(env);
      if(m.status==="closed")return json({ok:true,open:false,alreadyClosed:true,total:(m.codes||[]).length,submitted:await countVotes(env,m.id)});
      const votes=await loadVotes(env,m.id),rows=rowsFromVotes(votes),winners=winnersFromRows(rows),dyn=await getDynamicRanking(env,m.season);
      for(const w of winners){dyn[w.category]??={};dyn[w.category][w.player]=Number(dyn[w.category][w.player]||0)+1}
      m={...m,status:"closed",closedAt:new Date().toISOString()};
      await env.STATE.put(`ranking:${m.season}`,JSON.stringify(dyn));await env.STATE.put(`result:${m.id}`,JSON.stringify({rows,winners}));await putMatch(env,m);
      return json({ok:true,open:false,winners,total:(m.codes||[]).length,submitted:votes.length});
    }
    if(request.method==="POST"&&u.pathname==="/api/admin/open"){
      if(!admin(request,env))return json({error:"PIN de administrador incorreto."},401);
      let m=await getMatch(env);
      if(m.status==="open")return json({ok:true,open:true,alreadyOpen:true,total:(m.codes||[]).length,submitted:await countVotes(env,m.id)});
      const saved=await env.STATE.get(`result:${m.id}`,"json");
      if(saved?.winners?.length){const dyn=await getDynamicRanking(env,m.season);for(const w of saved.winners){dyn[w.category]??={};const n=Math.max(0,Number(dyn[w.category][w.player]||0)-1);if(n)dyn[w.category][w.player]=n;else delete dyn[w.category][w.player]}await env.STATE.put(`ranking:${m.season}`,JSON.stringify(dyn));await env.STATE.delete(`result:${m.id}`)}
      m={...m,status:"open",closedAt:null};await putMatch(env,m);
      return json({ok:true,open:true,total:(m.codes||[]).length,submitted:await countVotes(env,m.id)});
    }
    if(request.method==="POST"&&u.pathname==="/api/admin/players"){
      ...
      const players=await getPlayers(env);let p=players.find(x=>x.name.toLocaleLowerCase("pt-BR")===name.toLocaleLowerCase("pt-BR"));
      if(p){p.name=name;p.active=true}else{p={id:Math.max(0,...players.map(x=>Number(x.id)||0))+1,name,active:true};players.push(p)}
      await putPlayers(env,players);return json({ok:true,player:p});
    }
    if(request.method==="POST"&&u.pathname==="/api/admin/players/toggle"){
      if(!admin(request,env))return json({error:"PIN de administrador incorreto."},401);
      const b=await request.json(),id=Number(b.id),players=await getPlayers(env),p=players.find(x=>Number(x.id)===id);
      if(!p)return json({error:"Jogador não encontrado."},404);p.active=!!b.active;await putPlayers(env,players);return json({ok:true});
    }
