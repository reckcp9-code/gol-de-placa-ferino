...
async function finalizeMatch(env,m){
  const rows=await resultRows(env,m.id),winners=winnerRows(rows);
  const statements=[
  ...
  await env.DB.batch(statements);return {rows,winners};
}
...
export default {async fetch(request,env){
  ...
  try{
    if(request.method==="GET"&&u.pathname==="/api/health"){
      const ping=await env.DB.prepare("SELECT 1 ok").first();return json({ok:Number(ping?.ok||0)===1,db:true});
    }
    if(request.method==="GET"&&u.pathname==="/api/players")return json(await playerList(env,true));
    if(request.method==="GET"&&u.pathname==="/api/status"){
      const m=await currentMatch(env);if(!m)return json({match:null,open:false,total:0,submitted:0});
    ...
    if(request.method==="POST"&&u.pathname==="/api/vote"){
      const m=await currentMatch(env);if(!m||m.status!=="open")return json({error:"A votação já foi encerrada. O resultado está liberado."},423);
      ...
      try{await env.DB.batch(CATS.map(c=>env.DB.prepare("INSERT INTO match_votes(match_id,code_id,category,player) VALUES(?,?,?,?)").bind(m.id,code.id,c,String(b.votes[c]).trim())))}catch{
        await env.DB.prepare("UPDATE match_codes SET used_at=NULL WHERE id=?").bind(code.id).run();return json({error:"Não foi possível registrar o voto. Tente novamente."},500);
      }
      return json({ok:true});
    }
    if(request.method==="GET"&&u.pathname==="/api/admin/status"){
      ...
      const m=await currentMatch(env),players=await playerList(env,false);
    ...
    if(request.method==="POST"&&u.pathname==="/api/admin/codes"){
      if(!admin(request,env))return json({error:"PIN de administrador incorreto."},401);
      const m=await currentMatch(env);if(!m||m.status!=="open")return json({error:"A votação está encerrada. Crie ou reabra uma partida antes de gerar códigos."},423);
    ...
    if(request.method==="POST"&&u.pathname==="/api/admin/close"){
      if(!admin(request,env))return json({error:"PIN de administrador incorreto."},401);
      const m=await currentMatch(env);if(!m)return json({error:"Nenhuma partida cadastrada."},404);
      if(m.status==="closed")return json({ok:true,open:false,alreadyClosed:true,...await matchStats(env,m.id)});
      const done=await finalizeMatch(env,m);return json({ok:true,open:false,winners:done.winners,...await matchStats(env,m.id)});
    }
    if(request.method==="POST"&&u.pathname==="/api/admin/open"){
      if(!admin(request,env))return json({error:"PIN de administrador incorreto."},401);
      const m=await currentMatch(env);if(!m)return json({error:"Nenhuma partida cadastrada."},404);
      if(m.status==="open")return json({ok:true,open:true,alreadyOpen:true,...await matchStats(env,m.id)});
      await env.DB.batch([env.DB.prepare("DELETE FROM season_awards WHERE match_id=?").bind(m.id),env.DB.prepare("UPDATE matches SET status='open',closed_at=NULL WHERE id=?").bind(m.id)]);
